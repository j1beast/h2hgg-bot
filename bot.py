...
USUARIOS_PERMITIDOS = [7339330267, 1021947497, 409760550, 1316315194, 1478076850, 7515654372, 6132030515]
                ...
                if partidos_a and partidos_b:
                    ...
                    if betsson_pred:
                        ...
                        ya_enviado = conn_c.execute('''SELECT enviado_canal FROM predicciones
                              WHERE ((jugador_a=? AND jugador_b=?) OR (jugador_a=? AND jugador_b=?))
                              AND datetime(fecha_prediccion) >= datetime('now', '-2 hours')
                              AND enviado_canal=1''',
                        ...
                        if hay_valor_ou and bs_linea and linea_bot:
                            try:
                                ...
                                try:
                                    ou_factors = {'h2h': analisis.get('ou_h2h_total'), 'reciente': analisis.get('ou_reciente'), 'contraataque_rel': analisis.get('ou_contraataque_rel'), 'tendencia_pts': analisis.get('ou_tendencia_pts'), 'defensa': ((analisis.get('ou_defensa_a') or 0) + (analisis.get('ou_defensa_b') or 0)) or None, 'total_hist': analisis.get('ou_total_hist'), 'ritmo_franq': analisis.get('ou_ritmo_franq'), 'tendencia_h2h': analisis.get('ou_tendencia_h2h')}
...
def calcular_pesos_optimos_ou():
    ...
    c.execute('''SELECT ou_h2h_total, ou_reciente,
             ou_contraataque_rel, ou_tendencia_pts, ou_total_hist, ou_ritmo_franq, ou_tendencia_h2h, linea_betsson_ou, pts_real_a, pts_real_b
                 FROM predicciones
...
def analizar_partido(jugador_a, franq_a, jugador_b, franq_b, partidos_h2h, partidos_a, partidos_b):
    ...
    if resultado["avg_pts_a"] and resultado["avg_pts_b"]:
        ...
        avg_total_hist_a = round(sum(p["pts_favor"] + p["pts_contra"] for p in partidos_a_ou) / len(partidos_a_ou), 1) if partidos_a_ou else None
        avg_total_hist_b = round(sum(p["pts_favor"] + p["pts_contra"] for p in partidos_b_ou) / len(partidos_b_ou), 1) if partidos_b_ou else None
        resultado["ou_total_hist"] = round((avg_total_hist_a + avg_total_hist_b) / 2, 1) if avg_total_hist_a and avg_total_hist_b else None
        partidos_a_franq_ou = [p for p in partidos_a_franq if p["pts_favor"] + p["pts_contra"] >= 40] if partidos_a_franq else []
        partidos_b_franq_ou = [p for p in partidos_b_franq if p["pts_favor"] + p["pts_contra"] >= 40] if partidos_b_franq else []
    ...
    if analisis.get('linea_total'):
        ...
        try:
            ou_factors = {
                ...
                'contraataque_rel': analisis.get('ou_contraataque_rel'),
...
async def optimizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pesos_ou:
        ...
        nombres_ou = {'h2h': 'H2H total', 'reciente': 'Forma reciente',
                      'contraataque_rel': 'Contraataque relativo', 'tendencia_pts': 'Tendencia de puntos',
        ...
        for k in ['h2h', 'reciente', 'contraataque_rel', 'tendencia_pts', 'total_hist', 'ritmo_franq', 'tendencia_h2h']:
            n = n_ou.get(k, 0)
            if n < 10:
